...
export class UsersController {
  ...
  @Patch('me/supplier-application')
  @ApiOperation({ summary: 'Apply to become a supplier' })
  @ApiOkResponse({ description: 'Supplier application is now pending admin review' })
  applyForSupplier(
    @Req() request: AuthenticatedRequest,
    @Body() dto: CreateSupplierApplicationDto,
  ): Promise<User> {
    return this.usersService.applyForSupplier(
  ...
  @Patch('me/onboarding')
  @ApiOperation({ summary: 'Complete or skip first-login onboarding' })
  @ApiOkResponse({ description: 'Onboarding preferences saved' })
  completeOnboarding(@Req() request: AuthenticatedRequest, @Body() dto: CompleteOnboardingDto): Promise<User> {
    return this.usersService.completeOnboarding(this.getCurrentUserId(request), dto);
  }
